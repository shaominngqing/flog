...
- **`domain/`** — Pure data types with zero UI dependencies
  ...
  - `sse_merge.rs` — SSE Merged View utilities: `extract_field_paths` (scans all chunks for leaf-string JSON paths), `resolve_path`, `auto_detect_field` (knows OpenAI/Codex streaming patterns), `merge_field` (concatenates a field across chunks)
