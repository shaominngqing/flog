...
async fn test_discover_and_connect_dds() {
    let dds_ws_url = match discover_dds_url().await {
        Some(url) => url,
        None => { println!("Could not discover DDS URL"); return; }
    };
        ...
        Ok(Ok((ws, resp))) => {
            ...
            loop {
                match tokio::time::timeout_at(deadline, rx.next()).await {
                    Ok(Some(Ok(Message::Text(t)))) => {
                        ...
                        if let Some(id) = json.get("id") {
                            ...
                            if let Some(err) = error {
                                ...
                                let result_type = json.get("result")
                                ...
                                if id.as_str() == Some("vm1") {
                                    let name = json.pointer("/result/name")
                                        .and_then(|n| n.as_str())
                                        .unwrap_or("?");
                                    let version = json.pointer("/result/version")
                                        .and_then(|v| v.as_str())
                                        .unwrap_or("?");
                            ...
                            if stream_id == "Logging" {
                                let msg = json.pointer("/params/event/logRecord/message/valueAsString")
                                    .and_then(|s| s.as_str())
                                    .unwrap_or("(no message)");
                                let tag = json.pointer("/params/event/logRecord/loggerName")
                                    .and_then(|s| s.as_str())
                                    .unwrap_or("?");
                                println!("[event #{}] {} | {} | {} | {}", event_count, method, stream_id, tag, &msg[..msg.len().min(80)]);
                            } else {
                                println!("[event #{}] {} | {} | {}", event_count, method, stream_id, event_kind);
                            }
                        }
                    }
                    ...
                    Err(_) => {
                        println!("\nTimeout (15s). Messages: {}, Events: {}", msg_count, event_count);
                        break;
                    }
                }
            }

            println!("\n=== RESULT: Connection stable! {} msgs, {} events ===", msg_count, event_count);
        }
        Ok(Err(e)) => {
