...
pub fn draw_timeline(f: &mut Frame, app: &mut App, area: Rect) {
    ...
    if width == 0 || app.store.is_empty() {
        f.render_widget(
            Paragraph::new("").block(Block::default().borders(Borders::TOP).border_style(Style::default().fg(SURFACE0))).style(Style::default().bg(BASE)),
            area,
        );
        return;
    }
    ...
    for _ in 0..width {
        ...
        for _ in 0..bs {
            ...
            if let Some(entry) = app.store.get(idx) {
                c += 1;
                match entry.level { LogLevel::Error => e = true, LogLevel::Warning => w = true, _ => {} }
            }
            idx += 1;
        }
        buckets.push((c, e, w));
    ...
    for &(count, has_err, has_warn) in &buckets {
        let bi = if count == 0 { 0 } else { ((count as f64 / mx as f64) * 7.0) as usize };
        let color = if has_err { RED } else if has_warn { YELLOW } else { BLUE };
        bar_spans.push(Span::styled(BARS[bi.min(7)].to_string(), Style::default().fg(color).bg(BASE)));
    }

    let ft = app.store.get(0).map(|e| e.timestamp.clone()).unwrap_or_default();
    let lt = app.store.get(total.saturating_sub(1)).map(|e| e.timestamp.clone()).unwrap_or_default();

    let bar_line = Line::from(bar_spans);
    let time_line = Line::from(vec![
        Span::styled(format!(" {}", ft), Style::default().fg(OVERLAY0)),
        Span::styled(format!("{:>w$}", format!("{} ", lt), w = width.saturating_sub(ft.len() + 1)), Style::default().fg(OVERLAY0)),
    ]);

    f.render_widget(
        Paragraph::new(vec![bar_line, time_line])
            .block(Block::default().borders(Borders::TOP).border_style(Style::default().fg(SURFACE0)))
...
pub fn click_to_offset(x: u16, total_width: u16, filtered_count: usize) -> usize {
    ...
    if w <= 0.0 || filtered_count == 0 { return 0; }
