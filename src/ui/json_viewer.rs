...
use super::{SURFACE0, OVERLAY0, BLUE, TEAL, GREEN, YELLOW, PEACH, MAUVE, PINK, SAPPHIRE, LAVENDER};
...
pub fn bracket_format(text: &str) -> Vec<FmtLine> {
    ...
    if let Some(sp) = ss {
        ...
        if !prefix.is_empty() {
            lines.push(FmtLine { text: prefix.to_string(), depth: 0, close_line: None });
        }
        let raw = indent_brackets(&text[sp..]);
        let base = lines.len();
        for r in &raw {
            lines.push(FmtLine { text: r.clone(), depth: 0, close_line: None });
        }
        ...
        for l in text.lines() {
            lines.push(FmtLine { text: l.to_string(), depth: 0, close_line: None });
        }
    }
    lines
}
...
pub fn init_state(fmt_lines: &[FmtLine], auto_expand_depth: usize) -> JsonViewerState {
    ...
    state.total_lines = fmt_lines.len();
...
fn colorize_value(text: &str) -> Vec<Span<'static>> {
    ...
    if v.starts_with('"') && v.ends_with('"') {
        ...
        spans.push(Span::styled(v.to_string(), Style::default().fg(NULL_COLOR).add_modifier(Modifier::ITALIC)));
        ...
        spans.push(Span::styled(v.to_string(), Style::default().fg(brace_color(0))));
    } else {
        spans.push(Span::styled(v.to_string(), Style::default().fg(STR_COLOR)));
    }
    if comma {
